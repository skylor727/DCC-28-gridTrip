...
const gridTrip = (xyCords, moves) => {
    const newCords = xyCords;
    const eachMove = moves.match(/[a-zA-Z][0-9]+/g);
    eachMove.forEach((move) => {
      let direction;
      if (move.charAt(0) === "U" || move.charAt(0) === "D") direction = 0;
      else direction = 1;
      let moveCount = move.match(/[0-9]+/g);
      if(move.charAt(0) === 'D' || move.charAt(0) === 'L') {
        newCords[direction] -= parseInt(moveCount)
      }
      else {
        newCords[direction] += parseInt(moveCount)
      }
